...
(function(w){
  ...
  w.republicstarSafeRedirect = function(url, fallback) {
    var dest = (typeof url === 'string') ? url.trim() : '';
    var fb   = fallback || '../GOVERN_1.HTM';
    if (!dest) return fb;
    if (/^(https?:)?\/\//i.test(dest)) return fb;
    if (/^javascript:/i.test(dest))    return fb;
    return dest;
  };
