...
function buildEmailHtml(toName, otpCode, toEmail) {
...
<table width="560" cellpadding="0" cellspacing="0" bgcolor="#ffffff"
    ...
    <td style="background:#0a0a0a;padding:24px 36px;border-bottom:4px solid #c8102e;">
      <table cellpadding="0" cellspacing="0"><tr>
        ...
        <td style="vertical-align:middle;">
          <div style="font-size:16px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#ffffff;line-height:1.2;">United Republic of Stars</div>
          <div style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#666666;margin-top:4px;">Citizen Portal &middot; Account Security</div>
        </td>
      </tr></table>
    </td>
  </tr>
  <tr>
    <td style="padding:36px 36px 28px;">
      ...
      <p style="font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#999999;margin:0 0 10px;">Your verification code</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;"><tr>
        <td style="background:#f7f7f7;border:1px solid #e0e0e0;border-left:4px solid #c8102e;border-radius:4px;padding:20px 24px;">
          ...
          <div style="font-size:12px;color:#999999;margin-top:10px;">Valid for <span style="color:#c8102e;font-weight:600;">5 minutes</span>.</div>
        </td>
      </tr></table>
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;"><tr>
        <td style="background:#fff8f8;border:1px solid #f5c0c8;border-radius:4px;padding:14px 18px;font-size:13px;color:#7a1a24;line-height:1.6;">
      ...
      <p style="font-size:14px;color:#555555;line-height:1.8;margin:0;">
        Kind regards,<br>
        <strong style="color:#0a0a0a;">Ministry of the Interior — United Republic of Stars</strong><br>
  ...
  <tr>
    <td style="background:#f7f7f7;border-top:1px solid #e8e8e8;padding:18px 36px;text-align:center;">
      <p style="font-size:11px;color:#aaaaaa;line-height:1.7;margin:0;">
...
async function sendEmail(toEmail, toName, otpCode) {
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender:      { name: 'United Republic of Stars', email: 'aziendalenational@gmail.com' },
      to:          [{ email: toEmail, name: toName }],
      subject:     'Your verification code — United Republic of Stars',
      htmlContent: buildEmailHtml(toName, otpCode, toEmail)
    })
  });
  if (!res.ok) {
    const text = await res.text().catch(() => res.statusText);
    console.error('Brevo error', res.status, text);
    throw new HttpsError('internal', 'Failed to send email: ' + res.status);
  }
}
