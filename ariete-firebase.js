/* ═══════════════════════════════════════════════════
   ARIETE FIREBASE  —  Shared Firestore backend
   United Republic of Stars
═══════════════════════════════════════════════════ */

/* ── FILL IN YOUR FIREBASE CONFIG HERE ── */
var ARIETE_FB_CONFIG = {
...
(function(){
  if (typeof firebase === 'undefined'){
    console.warn('[ArieteDB] Firebase SDK not ready — retrying in 300ms');
    setTimeout(arguments.callee, 300);
    return;
  }
  try {
    if (!firebase.apps.length) firebase.initializeApp(ARIETE_FB_CONFIG);
    ...
    console.log('[ArieteDB] Firestore connected ✓');
  } catch(e){
    console.error('[ArieteDB] Init error:', e);
  }
})();
